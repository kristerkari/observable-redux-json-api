...
const postsPayload = require("./payloads/linked_payload.json");
...
describe("[State mutation] Insertion of links", () => {
  ...
  it("should read and insert links into state when indexLinks string is present", () => {
    ...
    expect(updatedState.links.posts.self).toEqual(
      "http://api-host/api_path/posts?page%5Bnumber%5D=1&page%5Bsize%5D=2"
    );
    expect(updatedState.links.posts.next).toEqual(
      "http://api-host/api_path/posts?page%5Bnumber%5D=2&page%5Bsize%5D=2"
    );
  });
});
