...
import { IS_UPDATING, reducer } from "../src/jsonapi";
...
const topics = require("./payloads/topics.json");
...
describe("[State Mutation] Update or Reverse relationships", () => {
  it("Should update a resource relationship", () => {
    ...
    expect(updatedEntities[0].relationships.task.data).toEqual({
      id: resource.id,
      type: resource.type
    });
  });
  ...
  it("Should not mutate relationship, if new relationship is not null", () => {
    ...
    expect(updatedEnteties[2].relationships.task.data).toBe(
      state.transactions.data[2].relationships.task.data
    );
  });
