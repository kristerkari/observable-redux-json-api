...
Object.defineProperty(global, "XMLHttpRequest", {
  value: require("xmlhttprequest").XMLHttpRequest
});
...
const multipleResources = {
  data: [taskWithTransaction.data],
  relationships: [taskWithTransaction.data.relationships]
};
...
describe("Reading resources", () => {
  ...
  it("should append included resources in state", () => {
    ...
    expect(updatedState.transactions.data.length).toEqual(
  ...
  it("should handle response where data is an object", () => {
    const updatedState = reducer(
      undefined,
      apiRead(responseDataWithSingleResource)
    );
  ...
  it("should handle response with a one to many relationship", () => {
    const updatedState = reducer(
      state,
      apiRead(responseDataWithOneToManyRelationship)
    );
    expect(typeof updatedState.users).toBe("object");
    expect(typeof updatedState.companies).toBe("object");
    expect(
      Array.isArray(updatedState.users.data[0].relationships.companies.data)
    ).toBe(true);
  });
  ...
  it("should ignore reverse relationship with no matching resource", () => {
    const updatedState = reducer(
      state,
      apiRead(payloadWithNonMatchingReverseRelationships)
    );

    payloadWithNonMatchingReverseRelationships.included
      .filter(resource => resource.type === "reports")
      .forEach(payloadReport => {
        const stateReport = updatedState.reports.data.find(
          r => payloadReport.id === r.id
        );
        expect(stateReport.relationships.file.data.id).toEqual(
          payloadReport.relationships.file.data.id
        );
      });
  });
});
...
describe("Updating resources", () => {
  ...
  it("should persist in state and preserve order", () => {
    ...
    expect(state.users.data[0].attributes.name).not.toEqual(
      updatedUser.data.attributes.name
    );
    expect(updatedState.users.data[0].attributes.name).toEqual(
      updatedUser.data.attributes.name
    );
    zip([updatedState.users.data, state.users.data]).forEach((a, b) =>
  ...
  it("should be able to update a resource before type is in state", () => {
    ...
    const stateWithResourceType = reducer(
      stateWithoutUsersResource,
      apiWillUpdate(userToUpdate)
    );
    const updatedState = reducer(
      stateWithResourceType,
      apiUpdated(updatedUser)
    );
...
describe("Delete resources", () => {
  ...
  it("should remove reverse relationship", () => {
    ...
    expect(
      stateWithTask.transactions.data[0].relationships.task.data.type
    ).toEqual(taskWithTransaction.data.type);
    const stateWithoutTask = reducer(
      stateWithTask,
      apiDeleted(taskWithTransaction.data)
    );
    const {
      data: relationship
    } = stateWithoutTask.transactions.data[0].relationships.task;
  ...
  describe("when one-to-many relationship", () => {
    it("should update reverse relationship for transaction", () => {
      ...
      expect(
        stateWithTaskWithTransaction.transactions.data[0].relationships.task
          .data.type
      ).toEqual(taskWithTransactions.type);
    ...
    it("should update reverse relationship for task", () => {
      ...
      expect(stateWithTaskWithTransaction.transactions.data[0].id).toEqual(
        taskWithTransactions.relationships.transaction.data[0].id
      );

      const stateWithoutTransaction = reducer(
        stateWithTask,
        apiDeleted(transactionWithTask)
      );
      const {
        data: relationship
      } = stateWithoutTransaction.tasks.data[0].relationships.transaction;
...
describe("Endpoint values", () => {
  ...
  it("should update provided header, such as an access token", () => {
    ...
    expect(state.endpoint.headers).not.toEqual(header);
  ...
  it("should update to provided custom headers", () => {
    ...
    expect(state.endpoint.headers).not.toEqual(headers);
...
describe("Invalidating flag", () => {
  ...
  it("should be removed after update", () => {
    ...
    expect(updatedState.users.data[0].isInvalidating).toBe(undefined);
  });
});

describe("apiRequest", () => {
  it("should parse the response body on success", done => {
    expect.assertions(1);
  ...
  it("should return Body object when content-type is invalid", done => {
    expect.assertions(2);
  ...
  it("should return error when response is 500", done => {
    ...
    nock("http://foo.com")
      .get("/fakeurl")
      .reply(500, null, { "Content-Type": "application/json" });
